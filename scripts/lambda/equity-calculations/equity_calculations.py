...
def create_and_write_status_json(
    ...
    # TODO: Test what happens when ACL isn't public read. API should have
...
def read_in_point_data(
    ...
    if file_key.endswith("geojson"):
        try:
            ...
            # Rename GEOID column if present to avoid errors
            if "GEOID" in point_data.columns:
                point_data.rename(columns = {"GEOID": "GEOID_urbaninstitute"}, inplace = True)
...
def read_in_dem_geo_data(
        file_key,
        estimate_col_names,
        margin_col_rename_dict,
        geo_id_col,
        status_json,
        system_key,
        bucket,
    ...
    Args:
        file_key (str): File key on S3 (should be parsed directly from S3
            event).
        estimate_col_names (list): List with name of the column in the CSV to be used in analysis.
        margin_col_rename_dict (dict): Maps names of data columns (key - string) to their margin columns (value - string)
        geo_id_col (str): Name of column with geographic identifiers
        status_json (dict): Status JSON which will be updated and uploaded to S3
            as the function operates.
        system_key (str): system file key on S3
        bucket (str): bucket to write status_json to, and to read the CSV file
            from.
        df_type (str): either "demographic" or "geographic"
        n_char (int): Number of characters in GEOID.

    Raises:
        ValueError: Data Import from S3 not successful, when none of the 3 CSV
            encodings tried in the `read_in_csv_from_s3` function work
            to read in the file.
    ...
    if file_key.endswith("csv"):
        ...
        try:
            # Try reading in point data csv. It if fails, record
            # data_readin_error
            dem_geo_data = read_in_csv_from_s3(
                key = file_key,
                system_key = system_key,
                status_json = status_json,
                bucket = bucket,
    ...
    if file_key.endswith("geojson"):
        try:
            dem_geo_data_gdf = select_geojson_as_gdf(bucket, file_key)
            dem_geo_data = pd.DataFrame(dem_geo_data_gdf.drop(columns='geometry'))
        except:
            status_json["error-messages"]["data_readin_error"] = True
            update_status_json(system_key, status_json, bucket=bucket)
            raise ValueError("Data import from S3 not successful")
    ...
    # create list of all column names to check
    geo_id_col_new = geo_id_col + "_" + df_type
    num_col_names = [x + "_" + df_type for x in estimate_col_names] + list(margin_col_rename_dict.values())
    all_columns = [geo_id_col_new] + num_col_names

    # rename columns to follow naming convention
    estimate_col_rename_dict = {col : col + "_" + df_type for col in estimate_col_names}
    
    dem_geo_data.rename(columns = {geo_id_col : geo_id_col_new}, inplace = True)
    dem_geo_data.rename(columns = estimate_col_rename_dict, inplace = True)
    dem_geo_data.rename(columns = margin_col_rename_dict, inplace = True)

    # Try to subset data to just specified columns. If get key error because data
    # does not have all columns, raise ValueError
    try:
        dem_geo_data = dem_geo_data[all_columns]
    except:
        status_json["error-messages"][f"{df_type}_cols_not_in_data"] = True
        status_json["updates"]["error-messages"] = True
        update_status_json(
            system_key=system_key, status_json=status_json, bucket=bucket
        )
        raise ValueError(f"provided {df_type} columns not in data")


    # If geo_id_col isn't n_char characters for the geography FIPS, left pad with 0
    dem_geo_data[geo_id_col_new] = dem_geo_data[geo_id_col_new].str.pad(width = n_char, side = "left", fillchar = "0")

    # Try converting geographic/demographic columns to numeric. If we can't raise ValueError
    try:
        dem_geo_data[num_col_names] = dem_geo_data[num_col_names].apply(pd.to_numeric)
        return dem_geo_data
    except:
        status_json["error-messages"][
            f"{df_type}_cols_cannot_be_converted_numeric"
        ] = True
        status_json["updates"]["error-messages"] = True
        update_status_json(
            system_key=system_key, status_json=status_json, bucket=bucket
        )
        raise ValueError(f"{df_type}_cols_cannot_be_converted_numeric")
...
def convert_coltypes_for_filter_and_weights(
    ...
    # Try converting the filter columns into the selected column types if filters provided.
    # If we can't for any of the filters, error out and record in status_json.
    if filters:
        for x in filters:
...
def drop_null_filters_and_weights(
    ...
    if filters:
        ...
        num_filter_rows_dropped = status_json["warnings"]["num_null_filter_rows_dropped"]
    else:
        # if no filters specified, set num_filter_rows_dropped to 0 for calculation below
...
def try_readin_with_diff_encodings(s3_object, col_types = None):
    ...
    Args:
        ...
        col_types (dict): Optional dictionary of column data types
    ...
    """
    try:
        df = pd.read_csv(io.BytesIO(s3_object["Body"].read()), encoding="utf-8", dtype = col_types)
        return df
    except:
        try:
            df = pd.read_csv(
                io.BytesIO(s3_object["Body"].read()), encoding="ISO-8859-1", dtype = col_types
            )
            return df
        except:
            try:
                df = pd.read_csv(
                    io.BytesIO(s3_object["Body"].read()), encoding="utf-16", dtype = col_types
                )
                return df
            except:
                raise ValueError(
...
def read_in_csv_from_s3_as_gdf(
    ...
    # If column GEOID_urbaninstitute in file, raise error
    if "GEOID_urbaninstitute" in df.columns:
        status_json["error-messages"]["invalid_column_in_data"] = True
        status_json["updates"]["error-messages"] = True
        update_status_json(
            system_key=system_key, status_json=status_json, bucket=bucket
        )
        raise ValueError("Cannot use columnname 'GEOID_urbaninstitute'")

    # Rename GEOID column if present to avoid errors
    if "GEOID" in df.columns:
        df.rename(columns = {"GEOID": "GEOID_urbaninstitute"}, inplace = True)
...
def read_in_csv_from_s3(
    ...
    """Read in CSV file from S3 with demographic or geographic data
    ...
    Args:
        ...
        status_json (dict): status json to update with warnings/errors.
        bucket (str): bucket to write status_json to.
        all_columns (list): list with all column names required to be in csv
        df_type(str): one of "demographic" or "geographic"

    Raises:
        ValueError: "No appropriate CSV encoding. CSV can't be read in", when
            of the 3 default CSV encodings work to read in the file.
        ValueError: "provided [demographic/geographic] columns not in data", when given columns
            are not in the data.
        ValueError: "[demographic/geographic] columns cannot be converted to numeric", when given
            columns can't be converted to a numeric column.

    Returns:
        df (dataframe): A pandas dataframe of the geographic or demographic data
    """
    try:
        s3 = boto3.client("s3", region_name=data_bucket_region)
        obj = s3.get_object(Bucket=bucket, Key=key)
        df = try_readin_with_diff_encodings(obj, col_types = {geo_id_col: object})
...
def calculate_demo_disparity_score_and_sig_diff(
    ...
    if not np.isnan(summary_value_margin):
        ...
        test_statistic_den = math.sqrt(
            # TODO: Looks like we're using SD and margin interchangably?? Need to
        ...
        data_value_sd = np.nan
        summary_value_margin = np.nan
        sig_diff = False #Set to False on logic that we do not have evidence to reject the null hypothesis
...
def create_temp_geo(temp, geo, geo_disp, n_char, bucket, polygon_geoid, year):
    ...
    # TODO: Add check that number of rows in temp_geo[['agg_geo`]] are <= number
...
def minify(df, df_name):
    ...
    df = df.rename(columns = minify_dict[df_name])
...
def impute_missing_dem_base(temp, col_dict, df_type, status_json):
    # identify number of missing observations in each column
    col_names =list(col_dict.keys()) + list(col_dict.values())
    col_names = [col for col in col_names if col is not None]

    num_missing = temp[col_names].isna().sum().reset_index()
    num_missing.columns = ["col_name", "n_missing"]
    num_missing["pct_missing"] = num_missing["n_missing"] / temp.shape[0]
    #List all columns with over 50% missingness 
    missing_over_half = num_missing[num_missing["pct_missing"] > .5]["col_name"].to_list()
    print("missing over half is:", missing_over_half)

    cols_to_drop = []

    if len(missing_over_half) > 0:
        #List all margin columns where variable column is missing over half
        missing_over_half_values_from_variables = [col_dict[col] for col in missing_over_half if col in col_dict.keys() if col_dict[col]]
        
        #Combine previous lists to get all columns to be dropped:
        #  variables over 50% missingness, margin columns associated with those variables, and margin columns with over 50% missingness
        cols_to_drop = list(set(missing_over_half + missing_over_half_values_from_variables))
        print("Cols to drop are:", cols_to_drop)

        temp.drop(axis = 1, columns = cols_to_drop, inplace = True)
        status_json["warnings"][f"{df_type}_dropped_cols_over_half_missing_values"] = cols_to_drop
        # remove dropped columns from col_list

    status_json["warnings"][f"{df_type}_cols_any_missing_values"] = num_missing[num_missing["pct_missing"] > 0]["col_name"].to_list()

    # remove dropped key-value pairs from dictionary
    
    #Logic of lists and loops below is to identify whether to drop a key and value or just change the value to None
    variables_to_drop = []
    variables_with_margins_to_drop = []
    
    for column_name in cols_to_drop:
        if column_name.endswith("margin"): #dealing with a margin column name
            var = column_name[:-7]
            if var in cols_to_drop:
                variables_to_drop.append(var)
            else:
                variables_with_margins_to_drop.append(var) 
        else:
            variables_to_drop.append(column_name) 
    
    for variable in variables_to_drop:
        col_dict.pop(variable, None)
    
    for variable in variables_with_margins_to_drop:
        col_dict[variable] = None

    # impute missing estimates with 0
    # keep missing margins of error as missing
    temp[list(col_dict.keys())] = temp[list(col_dict.keys())].fillna(0)

    return temp, col_dict

def test_valid_dem_geo_cols(temp, col_dict, df_type, status_json):
    """
    For the user-specified demographic or geographic columns, test if for any tract, the estimate
    is greater than the total population. If so, impute with total population. This also results
    in tracts with 0 total population imputing all estimates to 0.
    """

    more_than_half_greater_total_pop = []
    any_greater_total_pop = []
    more_than_half_negative = []
    any_negative = []
    any_negative_margin = []
    any_float_values = []
    ...
    for col in col_dict.keys():
        ...
        # assess share of values where count value is larger than the total population for the tract
        # cols with more than half of values meeting this condition are dropped
        pct_greater_than_pop =  temp[temp[col] > temp["num_pop"]].shape[0]/ temp.shape[0]

        if pct_greater_than_pop > .5:
            more_than_half_greater_total_pop.append(col)
            any_greater_total_pop.append(col)

        elif pct_greater_than_pop > 0:
            # impute vals greater than total pop with total pop
            temp[col] = np.where(temp[col] > temp["num_pop"], temp["num_pop"], temp[col])
            any_greater_total_pop.append(col)

        # assess share of values where count value is negative. cols with more than half of values
        # meeting this condition are dropped
        pct_neg = temp[temp[col] < 0].shape[0] / temp.shape[0]

        if pct_neg > .5:
            more_than_half_negative.append(col)
            any_negative.append(col)

        elif pct_neg > 0:
            # impute negatives with 0
            temp[col] = np.where(temp[col] < 0, 0, temp[col])
            any_negative.append(col)

        # assess if column has float values
        if not np.array_equal(temp[col], temp[col].astype(int)):
            any_float_values.append(col)

        # if margin column is not None, perform checks for margin column
        col_margin = col_dict[col]
        if col_margin:
            pct_greater_than_pop_margin =  temp[temp[col_margin] > temp["num_pop"]].shape[0]/ temp.shape[0]
            # we don't impute margin values greater than total population because this can be valid for highly uncertain
            # estimates in small populations, we do add column to warning for user awareness
            if pct_greater_than_pop_margin > 0:
                any_greater_total_pop.append(col_margin)
            
            # assess share of values where count value is negative. cols with more than half of values
            # meeting this condition are dropped
            pct_neg_margin = temp[temp[col_margin] < 0].shape[0] / temp.shape[0]

            # if the margin column has any negative values, we add the col to the list to be dropped and replace
            # margin entry in the col_dict with None
            if pct_neg_margin > 0:
                col_dict[col] = None
                any_negative_margin.append(col_margin)

            # assess if margin column has float values
            #Note: add fillna(0) to avoid warnings with missing values and np.ceil() to avoid case where decimals all round down
            if not np.array_equal(temp[col_margin].fillna(0), np.ceil(temp[col_margin].fillna(0)).astype(int)):
                any_float_values.append(col_margin)


    # drop columns with more than half invalid columns and update col_dict
    all_cols_drop = more_than_half_greater_total_pop + more_than_half_negative
    all_cols_drop_values = [col_dict[col] for col in all_cols_drop if col_dict[col]]
    # create list of unique columns to be dropped
    all_cols_drop_all = list(set(all_cols_drop + all_cols_drop_values + any_negative_margin))
    temp.drop(columns = all_cols_drop_all, axis = 1, inplace = True)

    # remove dropped key-value pairs from dictionary
    for key in all_cols_drop:
        col_dict.pop(key, None)

    # update warning messages
    status_json["warnings"][f"{df_type}_dropped_over_half_values_greater_than_total_pop"] = more_than_half_greater_total_pop
    status_json["warnings"][f"{df_type}_values_greater_than_total_pop"] = any_greater_total_pop
    status_json["warnings"][f"{df_type}_dropped_over_half_values_negative"] = more_than_half_negative
    status_json["warnings"][f"{df_type}_dropped_any_values_negative_margin"] = any_negative_margin
    status_json["warnings"][f"{df_type}_values_negative"] = any_negative
    status_json["warnings"][f"{df_type}_float_values"] = any_float_values

    #AS: this is from the prototype, but I imagine that this issue would be solved by first row
    #temp[col] = np.where(temp["num_pop"] == 0, 0, temp[col])
    # TODO: for columns in ACS, do we already do this correction or is it handled here?

    return temp, col_dict

def add_pct_prop_cols(temp, cols_dict, df_type, status_json):
    for est_col, moe_col in cols_dict.items():
        temp["prop_" + est_col] = temp[est_col] / temp[est_col].sum()
        temp["pct_" + est_col] = temp[est_col] / temp["num_pop"] * 100

        if moe_col:
            #Calculate the moe for the percent
            #Equation 3 page 6:https://www2.census.gov/programs-surveys/acs/tech_docs/accuracy/2019_ACS_Accuracy_Document_Worked_Examples.pdf
            X_pct = temp[est_col]
            Y_pct = temp["num_pop"]
            Se_X_pct = temp[moe_col] / 1.645
            Se_Y_pct = temp["num_pop_margin"] / 1.645
            temp["pct_" + est_col + "_margin"] = 1/Y_pct * np.sqrt(Se_X_pct**2 - X_pct ** 2 / Y_pct ** 2 * Se_Y_pct ** 2) * 100

            #Calculate the moe for the proportion of total (using same equation as above)
            X_prop = temp[est_col]
            Y_prop = temp[est_col].sum()
            Se_X_prop = temp[moe_col] / 1.645
            #Equation 2 from page 5: https://www2.census.gov/programs-surveys/acs/tech_docs/accuracy/2019_ACS_Accuracy_Document_Worked_Examples.pdf
            Se_Y_prop = np.sqrt(((temp[moe_col]/1.645) **2).sum())
            temp["prop_" + est_col + "_margin"] = 1 / Y_prop * np.sqrt(Se_X_prop ** 2 - X_prop ** 2 / Y_prop ** 2 * Se_Y_prop ** 2)

        else:
            status_json["warnings"][f"{df_type}_missing_moe_columns"] = True
            temp["pct_" + est_col + "_margin"] = np.nan
            temp["prop_" + est_col + "_margin"] = np.nan

    return temp

def update_geoid_col_name(col_name):
    """If user-provided colname is GEOID, convert to GEOID_urbaninstitute

    Args:
        col_name(str or list): name or column or filter dictionary list
    """
    ...
    if isinstance(col_name, list):
        for d in col_name:
            if d["filter_column"] == "GEOID":
                d["filter_column"] = "GEOID_urbaninstitute"
    else:
        if col_name == "GEOID":
            col_name = "GEOID_urbaninstitue"
...
def handler(event, context):
    ...
    ## Args for testing script
    # file_key = "sample-data/us_library_outlets.csv"
    # file_key = "input-data/1636642230-431456.csv"
    # trigger_bucket = "ui-equity-tool-prod"
    # request_id = 10
    ...
    # set of parameters that is only included in user API event
    try:
        is_public_api = event["is_public_API"]
        has_demographic = event["has_demographic_data"]
        has_geographic = event["has_geographic_data"]

        print("Parsing user API fields...")
        print("is_pubic_api: ", is_public_api)
        print("has_demographic: ", has_demographic)
        print("has_geographic: ", has_geographic)
        ...
        raise ValueError("unable to access user API parameters successfully")
    ...
    # the system key is the file name after the subdirectory and before the file
    # extension
    key_list = file_key.split("/")
    system_key = key_list[1].split(".")[0]

    # create file keys for form, resource, demographic, and geographic data

    if is_public_api:
        resource_file_key = "input-data/" + system_key + "_resource.csv"
    else:
        resource_file_key = "input-data/" + system_key + ".csv"
    
    form_key = "form-data/" + system_key + ".json"
    dem_file_key = "input-data/" + system_key + "_demographic.csv"
    geo_file_key = "input-data/" + system_key + "_geographic.csv"
    ...
    if is_public_api:
        year = unquote_plus(event["acs_data_year"])
        
        if year not in ["2022", "2021", "2019"]:
            year = "2022" 
    else:
        year = "2022"
    ...
    if is_public_api:
        try:
            lat_name = update_geoid_col_name(form_data["resource_lat_column"])
            lon_name = update_geoid_col_name(form_data["resource_lon_column"])
            filters = update_geoid_col_name(form_data["resource_filters"])
            weight_name = update_geoid_col_name(form_data["resource_weight"])
            geo = form_data["geo"]
        except:
            status_json["error-messages"][
    ...
    if has_demographic:
        try:
            dem_geo_id = form_data["demographic_geo_id_column"]
            dem_col_dict_raw = form_data["demographic_columns"]
            # create two dictionaries, one mapping margin column names to new name that follows
            # naming convention to work with demographic disparity score calculation, the other
            # with the updated names after renaming
            dem_margin_col_rename = {}
            dem_col_dict = {}
            for k in dem_col_dict_raw.keys():
                if dem_col_dict_raw[k]:
                    dem_margin_col_rename[dem_col_dict_raw[k]] = k + "_demographic_margin"
                    dem_col_dict[f"{k}_demographic"] = k + "_demographic_margin"
                else:
                    dem_col_dict[f"{k}_demographic"] = None
        except:
            status_json["error-messages"][
                "form-data-parameter-validation-failed"
            ] = True
    ...
    if has_geographic:
        try:
            geo_geo_id = form_data["geographic_geo_id_column"]
            geo_col_dict_raw = form_data["geographic_columns"]
            # create two dictionaries, one mapping margin column names to new name that follows
            # naming convention to work with geographic disparity score calculation, the other
            # with the updated names after renaming
            geo_margin_col_rename = {}
            geo_col_dict = {}
            for k in geo_col_dict_raw.keys():
                if geo_col_dict_raw[k]:
                    geo_margin_col_rename[geo_col_dict_raw[k]] = k + "_geographic_margin"
                    geo_col_dict[f"{k}_geographic"] = k + "_geographic_margin"
                else:
                    geo_col_dict[f"{k}_geographic"] = None
        except:
            status_json["error-messages"][
                "form-data-parameter-validation-failed"
            ] = True
    ...
    # weight columns. User API will pass None value. If either condition is the case, 
    # create a weight column and assign weight of 1 for all datapoints
    if weight_name == "" or not weight_name:
    ...
    # Read in geographic data from S3 if relevant
    if has_geographic:
        #geo_col_estimates = list(geo_col_dict.keys())
        geo_col_estimates = list(geo_col_dict_raw.keys())

        # non-null margins of error
        geo_col_margins = [x for x in geo_col_dict.values() if x]
        geo_columns = geo_col_estimates + geo_col_margins
        geographic_data = read_in_dem_geo_data(
            geo_file_key,
            geo_col_estimates,
            geo_margin_col_rename,
            geo_geo_id,
    ...
    read_in_geo_data_time = time.perf_counter()
    print("Read in geographic data")

    # Read in demographic data from S3 if relevant
    if has_demographic:
        dem_col_estimates = list(dem_col_dict_raw.keys())
        demographic_data = read_in_dem_geo_data(
            dem_file_key,
            dem_col_estimates,
            dem_margin_col_rename,
            dem_geo_id,
            ...
            bucket=trigger_bucket,
            df_type = "demographic",
            # demographic data is always tract level w/ 11-digit FIPS
            n_char = 11
        )

    read_in_dem_data_time = time.perf_counter()
    print("Read in demographic data")
    ...
    # TODO: we may want to project to a 2d/projected coord systems
    # TODO: for more accurate join results.
    ...
    # Join in supplemental geographic data if provided to temp_geo. We create separate joined data frames in case where
    # geographic and demographic dataframes have overlapping column names. This is used for geographic disparity
    # calculations. 
    if has_geographic:
        temp_geo = pd.merge(temp_geo, geographic_data, left_on = "GEOID", right_on = geo_geo_id + "_geographic", how = "left")
        # Assess prop missing values and impute with 0
        temp_geo, geo_col_dict_keep = impute_missing_dem_base(temp_geo, geo_col_dict, "geographic", status_json)

        # if there are still new cols that are valid
        if len(geo_col_dict_keep) > 0:
            #Address any areas where count is greater than num_pop for the are by converting the count to num_pop
            temp_geo, geo_col_dict_keep = test_valid_dem_geo_cols(temp_geo, geo_col_dict_keep, "geographic", status_json)

        # if there are still new cols that are valid
        if len(geo_col_dict_keep) > 0:
            #Add in pct_, and prop_ columns and rename count columns:
            temp_geo = add_pct_prop_cols(temp_geo, geo_col_dict_keep, "geographic", status_json)

    # Join in supplemental demographic data if provided to temp. This is used for demographic disparity calculations.
    if has_demographic:
        temp = pd.merge(temp, demographic_data, left_on = "GEOID", right_on = dem_geo_id + "_demographic", how = "left")

        # Assess prop missing values and impute with 0
        temp, dem_col_dict_keep = impute_missing_dem_base(temp, dem_col_dict, "demographic", status_json)

        # if there are still new cols that are valid
        if len(dem_col_dict_keep) > 0:
            #Address any areas where count is greater than num_pop for the are by converting the count to num_pop
            temp, dem_col_dict_keep = test_valid_dem_geo_cols(temp, dem_col_dict_keep, "demographic", status_json)

        # if there are still new cols that are valid
        if len(dem_col_dict_keep) > 0:
            #Add in pct_, and prop_ columns and rename count columns:
            temp = add_pct_prop_cols(temp, dem_col_dict_keep, "demographic", status_json)

    update_status_json(
        system_key=system_key, status_json=status_json, bucket=trigger_bucket
    )
    join_dem_geo_time = time.perf_counter()
    print("Join demographic and geographic data if provided")
    ...
    if has_geographic:
        baseline_cols = baseline_cols + ["prop_" + c for c in geo_col_dict_keep.keys()]
    ...
    # public-read ACLs
    # TODO: Figure out more secure way to serve data to user
    ...
    precomputed_stats_row = precomputed_stats.head(1)[["GEOID", "disp_name", "disp_name_abbv"]]

    tot_pop = temp["num_pop"].sum()
    #Equation 2 from page 5: https://www2.census.gov/programs-surveys/acs/tech_docs/accuracy/2019_ACS_Accuracy_Document_Worked_Examples.pdf
    se_tot_pop = np.sqrt(((temp["num_pop_margin"]/1.645) **2).sum())

    if has_demographic:
        for var in dem_col_dict_keep.keys():
            precomputed_stats_row_temp = precomputed_stats_row
            geog_wide_percentage = np.sum(temp["prop_pop"] * temp["pct_" + var])
            precomputed_stats_row_temp["var"] = var
            precomputed_stats_row_temp["value"] = geog_wide_percentage

            if dem_col_dict_keep[var]:
                se_tot_var = np.sqrt(((temp[var + "_margin"]/1.645) **2).sum()  ) #Equation 2 from page 5
                sum_var = temp[var].sum()

                #Equation 3 page 6:https://www2.census.gov/programs-surveys/acs/tech_docs/accuracy/2019_ACS_Accuracy_Document_Worked_Examples.pdf
                se_pct = 1/tot_pop * np.sqrt((se_tot_var)**2 - sum_var**2 / tot_pop **2 * se_tot_pop**2)
                margin_var_pct = se_pct * 1.645 * 100
                precomputed_stats_row_temp["value_margin"] = margin_var_pct
            else:
                precomputed_stats_row_temp["value_margin"] = np.nan

            precomputed_stats_row_temp = precomputed_stats_row_temp[list(precomputed_stats.columns)]
            precomputed_stats = pd.concat([precomputed_stats, precomputed_stats_row_temp], axis = 0)
    ...
    if not is_public_api and geo in ["national", "state"]:
    ...
    # # TODO: Change this in the R data download script so we don't do this in the
    ...
    # # diploma). This is just 1 - pct_hs_or_higher so we change the summary_value
    ...
    # stats_df.loc[
    #     stats_df["census_var"] == "pct_hs_or_higher", "summary_value"
    # ] = (
    #     1
    #     - stats_df.loc[
    #         stats_df["census_var"] == "pct_hs_or_higher", "summary_value"
    ...
    # For the GUI, minify column names to limit size of API response
    if not is_public_api:
        ...
        all_cols_geo = cols + ["weighted_counts"]
        temp_geo_cols = temp_geo[all_cols_geo]
        temp_geo_cols.rename(columns = {"GEOID": "num_geoid"}, inplace = True)
    ...
    # For the API, rename the temp_geo_cols geoid col to GEOID for user clarity
    else:
        temp_geo_cols.rename(columns = {"num_geoid": "GEOID"}, inplace = True)
    ...
    if not is_public_api and geo in ["state", "national"]:
        unique_sub_geo_json = make_unique_sub_geo_json(temp_geo, geo)
    else:
        unique_sub_geo_json = []

    #Create Presigned URLs for geobias csv and geojson and demographic bias csv
    #Note that the expiration time is arbitrarily set to be 10 minutes 
    s3 = boto3.client("s3", region_name=file_bucket_region)

    expiration_time = 60 * 10 
    try:
        geo_geojson_url = s3.generate_presigned_url(ClientMethod = "get_object",
                                                     Params ={
                                                         "Bucket":trigger_bucket,
                                                         "Key":geojson_key
                                                     },
        ...
        geo_csv_url = s3.generate_presigned_url(ClientMethod = "get_object", 
                                                     Params ={
                                                         "Bucket":trigger_bucket,
                                                         "Key":geo_csv_key
                                                     },
        ...
        demo_csv_url = s3.generate_presigned_url(ClientMethod = "get_object", 
                                                     Params ={
                                                         "Bucket":trigger_bucket,
                                                         "Key":demo_csv_key
                                                     },
        ...
        #Do we need this???
        # demo_json_url = s3.generate_presigned_url(ClientMethod = "get_object",
        #                                                Params = {
        #                                                    "Bucket":trigger_bucket,
        #                                                    "Key":demo_csv_key
        #                                                },
        ...
        status_json["error-messages"]["unable_to_generate_presigned_urls"] = True
        update_status_json(system_key, status_json, bucket=trigger_bucket)
        raise ValueError("Unable to generate presigned urls")
                                    

    download_links_dict = {
            "geo_bias_geojson":geo_geojson_url,
            "geo_bias_csv":geo_csv_url,
            "demographic_bias_csv":demo_csv_url
        }
    ...
    # access this file
    api_response_key = "output-data/api-response/" + system_key + ".json"
    ...
    logdata = pd.DataFrame(
        {
            ...
            "geo_readin_time":
                str(round(read_in_geo_data_time - start_time,2)),
            "dem_readin_time":
                str(round(read_in_dem_data_time - start_time,2)),
            "performed_sjoin_time":
                str(round(performed_spatial_join_time - start_time,2)),
            "join_dem_geo_time":
                str(round(join_dem_geo_time - start_time,2)),
